...
**2026-09-24 用户更新范围**：用户明确要求合并，并将大字号适配与专项测试延后。本轮移除6个重复的大字号用例，另将7个含独有功能断言的混合用例改为普通字号（深色、取消起盘、短拉不揭页、失败恢复、证据元数据与草稿往返继续测），同步CI选择；不撤销App已有动态字体支持。历史iOS 18大字号失败仍是未修，不改判通过。最新同源PR CI还出现普通字号报告入口失败，已据失败录屏单独修正转场等待，保留该回归并由新CI复验，不因字号范围调整忽略。实际合并状态、头提交和检查以[#9](https://github.com/wxbbb42/SUJI/pull/9)与Actions为准；本文件不预告未完成的合并。

当前产品说明和下一步见[产品状态摘要](2026-09-24-product-status.md)。R2仍是待确认方案，合并保存成果不等于批准实施。后续不把大字号专项作为当前阶段门槛；普通字号核心交互、账号/资料隔离和证据核验继续验收。
