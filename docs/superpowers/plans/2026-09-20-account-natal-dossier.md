...
- [ ] Engine snapshot round-trip: add `natal` command returning `schemaVersion`, `engineRevision`, `birthKey`, `calendarPolicy`, `mingPan`, `ziweiPan`, `personality`. Accept `natal` on profile/forecast/natal tools and first relationship chart. Validate identity and restore Date fields. Test no natal recalculation with spies on real engine methods, malformed snapshots, dynamic reference times and exact output parity.
- [ ] Durable native storage: add `NatalDossier` in SujiCore with owner, validated birth, bundle digest, createdAt and checksummed JSON payload. Store under separate `SavedState` key; coalesce concurrent requests. Test restart, corruption, account/birth/version mismatch and AppStore request routing.
- [ ] Account-first setup: replace guest onboarding cover with a root gate; show login, birth form, building/retry state, then tabs. Reuse account/password recovery UI. Automatically restore cloud birth only into an empty account; saved birth changes are queued for cloud upload with visible retry. Preserve user control over importing old local data.
- [ ] Verification: run engine typecheck/tests/build, Core and native integration tests, generate Xcode project, inspect diff and archive boundaries, update architecture notes, commit/push. macOS CI supplies native checks when local Xcode tooling is unavailable.
