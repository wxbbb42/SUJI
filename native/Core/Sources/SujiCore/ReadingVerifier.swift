...
public enum ReadingVerifier {
    ...
    referenceIndexVersion为1或2的索引按本条format还原每一个完整factKey、pointer、toolCallID；v2的stringNodes每连续2项是一行，patterns每连续4项是一行，其他数字序列与字符串片段语义相同。value从同轮完整工具按原pointer读取，先展开其字典和已核验来源目录。不存在的路径是缺证据，不能当作null、猜值或自行计算；编号序列仅压缩路径，不能当作爻位或日期结论。
    """
