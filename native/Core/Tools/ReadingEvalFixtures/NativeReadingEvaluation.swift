...
enum NativeReadingEvaluation {
    ...
    static func run() async throws {
        ...
        for item in config.cases {
            ...
            var record: [String: Any] = ["id": item.id, "question": item.question, "mode": mode, "birth": try object(birth), "context": try object(context)]
            ...
            do {
                let available = try ReadingIntent.definitions(from: definitions, mode: mode, question: item.question, hasBirth: birth != nil)
                let orchestrator = ToolOrchestrator(complete: { messages, tools in
                    try await complete(messages, tools: tools, phase: "planner")
                }, execute: { call in
            ...
            print("\(item.id): \(record["status"] ?? "unknown"), \(receipts.count) preserved receipts")
        }
    }
}
