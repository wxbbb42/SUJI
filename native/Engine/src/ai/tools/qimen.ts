...
import type { QimenSelectionRequest } from '@engine/qimen/specializedSelection';
...
const selectionRequestSchema = {
  type: 'object' as const, additionalProperties: false, required: ['focus'],
  description: '仅在用户明确要求天气或住宅专门取用时提出，须经原生确认；questionType使用event或general且event明确。只绑定原文支持的对象，不推断成败、天气实况或日期。',
  properties: {
    focus: {
      type: 'string',
      enum: ['weather-rain', 'weather-snow', 'weather-wind', 'weather-river-level',
        'dwelling-residence', 'dwelling-entrance', 'dwelling-stove', 'dwelling-tap-water',
        'dwelling-utensils', 'dwelling-courtyard', 'dwelling-skywell', 'dwelling-living-room'],
    },
  },
};
...
export const qimenTools: ToolDefinition[] = [
  {
    ...
    function: {
      name: 'setup_qimen',
      description: '为战略级重大决策起奇门盘。按本次提问物理时刻的北京时间标准时（UTC+08:00）起局，子初23:00换日；不借用出生地。返回拆补转盘9宫、值符值使、格局、时旬空覆盖、时马、门克宫及九星月令。questionContext保留明确对象/事件/远近；yongShen.candidates列日干、时干与类别参考，尚未定用，selectedCandidateId=null。每个occurrence有精确objectPath与地盘/天盘/寄干/中宫身份；甲保留原干和各自旬仪载体，中宫记录不是有效天盘。旧summary/state仅为单处干宫关系，不能代替候选或综合旺衰。旧类别候选映射是product-policy，非古籍已定断法。用户明确选择天气/住宅专门类别后，selectionRequest触发specializedSelection，按来源绑定角色及全部盘层；roleMappingEstablished只表示角色映射成立，不表示事件成败或已选定应期对象。yingQi保留未决参考；仅用户明确应期对象、事件时传timingRequest，由timing返回同一对象的空先马、马先墓刑冲的条件，明确单位及有界窗口后列日期。所有日期仅为条件候选，outcomeEstablished=false；缺少适用规则时不列日期，不能移植六爻应期、按宫数或远近编造期限。同一问题后补资料沿用原始盘与参数，不重起。ruleSources提供电子来源及边界。',
      parameters: {
        type: 'object',
        ...
        properties: {
          ...
          selectionRequest: selectionRequestSchema,
          gender: {
            type: 'string',
            enum: ['男', '女'],
...
export const qimenHandlers: Record<string, ToolHandler> = {
  setup_qimen: ({ question, questionType, gender, subject, event, timeHorizon, timingRequest, selectionRequest }, ctx) => {
    const chart = engine.setup({
      ...
      selectionRequest:selectionRequest as QimenSelectionRequest|undefined,
    });
    return chart;
  },
};
