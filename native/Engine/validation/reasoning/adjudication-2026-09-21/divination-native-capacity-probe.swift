...
@main struct Probe {
 static func main() async throws {
  ...
  for (index,fixture) in fixtures.enumerated() {
   ...
   let row:[String:Any]=["maxStoredReceiptBytes":result.receipts.map { $0.output.utf8.count }.max() ?? 0,"eventVariant":fixture.label ?? "plain-matrix","referenceRenderAccepted":reading != nil,"objectCount":objectCount,"renderedObjectCount":tombSections.count,"tracePointersExact":traceValid,"fanfuTracePointersExact":fanfuTraceValid,"triadTracePointersExact":triadTraceValid,"fanfuSections":fanfuSections.count,"renderRoundtrip":renderRoundtrip,"index":index,"date":fixture.date,"liveBytes":live.reduce(0){$0+($1.content?.utf8.count ?? 0)},"maxProjectedUTF16":projected.map(\.utf16.count).max()!,"maxToolUTF16":live.map{$0.content!.utf16.count}.max()!,"receiptCount":result.receipts.count,"capacityError":live.contains{($0.content ?? "").contains("容量不足")},"faithfulChartRoundtrip":faithful,"factsPreserved":facts(fullHistory)==facts(replay),"replayEqualsLive":live.map(\.content)==replay.filter{$0.role == .tool}.map(\.content),"retryEvidence":retried.evidence,"authenticated":authenticated,"sourceRemovalAccepted":sourceRemovalAccepted,"reviewUTF16":review.reduce(0){$0+($1.content?.utf16.count ?? 0)+($1.toolCalls ?? []).reduce(0){$0+ReadingVerificationEvidence.encoded($1.arguments).utf16.count}},"messageCount":review.count]
   for key in ["referenceRenderAccepted","tracePointersExact","fanfuTracePointersExact","triadTracePointersExact","renderRoundtrip","faithfulChartRoundtrip","factsPreserved","replayEqualsLive"] { precondition(row[key] as? Bool == true, "Failed native check: " + key) }
