...
for(let i=0;i<fixtures.length;i++){
 ...
 for(const message of request.messages){
  ...
  if(message.role==='system'&&message.content?.startsWith(sourcePrefix)){
   const directory=JSON.parse(message.content.slice(sourcePrefix.length));
  ...
  if(root.referenceIndexVersion!==undefined){
   assert.equal(root.referenceIndexVersion,1);assert.equal(root.valuesFromTool,true);
   const parts=[];
   assert.ok(Array.isArray(root.stringTokens)&&root.stringTokens.every(t=>typeof t==='string'));
   for(const node of root.stringNodes){
   ...
   for(const [idIndex,keyPartIndices,pointerPartIndices,sequenceIndex] of root.patterns){
