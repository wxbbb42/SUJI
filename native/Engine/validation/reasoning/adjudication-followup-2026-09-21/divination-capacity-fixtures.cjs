...
(async()=>{
 ...
 for (const focus of ['weather-rain','weather-snow','weather-wind','weather-river-level','dwelling-residence','dwelling-entrance','dwelling-stove','dwelling-tap-water','dwelling-utensils','dwelling-courtyard','dwelling-skywell','dwelling-living-room']) {
  const date='2004-07-02T08:00:00Z';
  const rows=[{name:'cast_liuyao',arguments:{question,questionType:'parents',subject:'parent',event,timeHorizon:'near'}},{name:'setup_qimen',arguments:{question,questionType:'event',subject:'self',event,timeHorizon:'far',selectionRequest:{focus},timingRequest:{focus:'self',event,timeUnit:'day',window:{end:'2100-12-31T23:59:59+08:00',maxCandidates:256}}}}];
  result.push({date,label:'specialized-selection-'+focus,rows:await run(date,rows,[6,6,6,6,9,9])});
 }
 fs.writeFileSync(folder+'/fixtures.json',JSON.stringify(result));console.log('Generated '+result.length+' actual-bundle paired inputs (48 original + five maximum-window + 12 specialized selection cases)');
