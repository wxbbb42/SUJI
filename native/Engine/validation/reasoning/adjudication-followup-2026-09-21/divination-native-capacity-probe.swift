...
@main struct Probe {
 static func main() async throws {
  let folder=URL(fileURLWithPath:CommandLine.arguments.count > 1 ? CommandLine.arguments[1] : "native/Engine/validation/reasoning/b5b-launch-acceptance/data").standardizedFileURL.path+"/"
  ...
  for (index,fixture) in fixtures.enumerated() {
   ...
   let projected=live.map { $0.content! }
   var entry=ConversationEntry(role:"user",text:String(repeating:"问",count:8000));entry.toolReceipts=result.receipts
   ...
   for (i,message) in live.enumerated() {
    var body=ToolOutputWire.decode(message.content!,name:calls[i].name,history:Array(result.messages.prefix { !($0.role == .tool && $0.toolCallID == calls[i].id) }),callID:calls[i].id) ?? .null
   ...
   let qimenReceipt=result.receipts.first{$0.name == "setup_qimen"}!
   let qimenReading=QimenReferenceReading.render(receipts:[qimenReceipt],context:context)
   ...
   let triadSections=reading?.sections.filter{$0.id.hasPrefix("triads-")} ?? []
   let triadTraceValid = !triadSections.isEmpty && triadSections.allSatisfy { section in !section.evidence.isEmpty && section.evidence.allSatisfy { $0.toolCallID == liuyaoReceipt.callID && $0.value == ReadingVerificationEvidence.pointer($0.pointer,in:liuyaoRoot) } }
   ...
   let row:[String:Any]=["maxStoredReceiptBytes":result.receipts.map { $0.output.utf8.count }.max() ?? 0,"eventVariant":fixture.label ?? "plain-matrix","referenceRenderAccepted":reading != nil,"qimenRenderAccepted":qimenReading != nil,"objectCount":objectCount,"renderedObjectCount":tombSections.count,"tracePointersExact":traceValid,"fanfuTracePointersExact":fanfuTraceValid,"triadTracePointersExact":triadTraceValid,"fanfuSections":fanfuSections.count,"renderRoundtrip":renderRoundtrip,"index":index,"date":fixture.date,"liveBytes":live.reduce(0){$0+($1.content?.utf8.count ?? 0)},"maxProjectedUTF16":projected.map(\.utf16.count).max()!,"maxToolUTF16":live.map{$0.content!.utf16.count}.max()!,"receiptCount":result.receipts.count,"capacityError":live.contains{($0.content ?? "").contains("容量不足")},"faithfulChartRoundtrip":faithful,"factsPreserved":facts(fullHistory)==facts(replay),"replayEqualsLive":live.map(\.content)==replay.filter{$0.role == .tool}.map(\.content),"retryEvidence":retried.evidence,"authenticated":authenticated,"sourceRemovalAccepted":sourceRemovalAccepted,"reviewUTF16":review.reduce(0){$0+($1.content?.utf16.count ?? 0)+($1.toolCalls ?? []).reduce(0){$0+ReadingVerificationEvidence.encoded($1.arguments).utf16.count}},"messageCount":review.count]
   try JSONSerialization.data(withJSONObject:row,options:[.prettyPrinted,.sortedKeys]).write(to:URL(fileURLWithPath:folder+"latest-case-report.json"))
   for key in ["qimenRenderAccepted","referenceRenderAccepted","tracePointersExact","fanfuTracePointersExact","triadTracePointersExact","renderRoundtrip","faithfulChartRoundtrip","factsPreserved","replayEqualsLive"] { precondition(row[key] as? Bool == true, "Failed native check: " + key) }
   precondition(row["capacityError"] as? Bool == false && authenticated == calls.count && sourceRemovalAccepted == 0)
   reports.append(row)
  }
