...
@main struct Probe {
 static func main() async throws {
  let fixtures=try JSONDecoder().decode([Fixture].self,from:Data(contentsOf:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/escaped-event-fixtures.json")))
  ...
  for (index,fixture) in fixtures.enumerated() {
   ...
   try JSONEncoder().encode(review).write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/escaped-capacity-request-\(index).json"))
   ...
   let row:[String:Any]=["eventVariant":fixture.label,"referenceRenderAccepted":reading != nil,"objectCount":objectCount,"renderedObjectCount":tombSections.count,"tracePointersExact":traceValid,"renderRoundtrip":renderRoundtrip,"index":index,"date":fixture.date,"liveBytes":live.reduce(0){$0+($1.content?.utf8.count ?? 0)},"maxToolUTF16":live.map{$0.content!.utf16.count}.max()!,"receiptCount":result.receipts.count,"capacityError":live.contains{($0.content ?? "").contains("容量不足")},"faithfulChartRoundtrip":faithful,"factsPreserved":facts(fullHistory)==facts(replay),"replayEqualsLive":live.map(\.content)==replay.filter{$0.role == .tool}.map(\.content),"retryEvidence":retried.evidence,"authenticated":authenticated,"sourceRemovalAccepted":sourceRemovalAccepted,"reviewUTF16":review.reduce(0){$0+($1.content?.utf16.count ?? 0)},"messageCount":review.count]
   reports.append(row)
  }
  let data=try JSONSerialization.data(withJSONObject:reports,options:[.prettyPrinted,.sortedKeys]);try data.write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/escaped-event-native-report.json"));print("Completed \(reports.count) native delivery/replay probes")
 }
}
