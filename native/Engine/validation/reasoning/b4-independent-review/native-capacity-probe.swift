...
@main struct Probe {
 static func main() async throws {
  let fixtures=try JSONDecoder().decode([Fixture].self,from:Data(contentsOf:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/capacity-fixtures.json")))
  ...
  for (index,fixture) in fixtures.enumerated() {
   ...
   try JSONEncoder().encode(review).write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/capacity-request-\(index).json"))
  ...
  let data=try JSONSerialization.data(withJSONObject:reports,options:[.prettyPrinted,.sortedKeys]);try data.write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b4-independent-review/native-capacity-report.json"));print("Completed \(reports.count) native delivery/replay probes")
 }
}
