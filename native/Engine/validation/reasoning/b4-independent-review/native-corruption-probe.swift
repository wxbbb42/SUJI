...
@main struct Probe {
 ...
 static func main() async throws {
  ...
  var variants:[(String,JSONValue)]=[
   ...
   ("valid-state-tamper",edit(packed,firstPath+"/1",ReadingVerificationEvidence.pointer(firstPath+"/1",in:packed) == .string("matched") ? "not-matched" : "matched")),
  ...
  for (name,v) in variants {
   ...
   results.append(["case":name,"expandAccepted":LiuyaoConditionTransport.expand(v) != nil,"factCount":facts(v).count,"originalFactCount":baseFacts,"authenticated":NatalEvidenceProjection.wasDelivered(receipt,in:history)])
  }
  ...
  for(name,v) in sourceVariants{
   ...
   results.append(["case":name,"renderAccepted":LiuyaoReferenceReading.render(receipts:[mutated],context:context) != nil])
  }
  let data=try JSONSerialization.data(withJSONObject:results,options:[.prettyPrinted,.sortedKeys]);try data.write(to:URL(fileURLWithPath:folder+"native-corruption-report.json"))
