...
@main struct Probe {
 static func main() async throws {
  ...
  for (index,fixture) in fixtures.enumerated() {
   ...
   try JSONEncoder().encode(review).write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b5a-independent-review/initial-capacity-request-\(index).json"))
   try JSONEncoder().encode(ReadingVerificationEvidence.facts(fullHistory)).write(to:URL(fileURLWithPath:folder+"initial-expected-facts-\(index).json"))
   CaptureProtocol.outputPath=folder+"initial-wire-request-\(index).json"
  ...
  let data=try JSONSerialization.data(withJSONObject:reports,options:[.prettyPrinted,.sortedKeys]);try data.write(to:URL(fileURLWithPath:"native/Engine/validation/reasoning/b5a-independent-review/initial-native-capacity-report.json"));print("Completed \(reports.count) native delivery/replay probes")
 }
}
