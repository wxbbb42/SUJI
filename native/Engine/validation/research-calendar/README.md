...
## Bounded 2057 boundary investigation

`hko-2057-investigate.py` compares the official 2057 PDF/TXT and one conjunction, preserving the original 200-year results. It records HKO's explicit warning for this exact new moon, checks the current PDF hash against the old audit, and separates ephemeris TT from delta-T using production lunar-javascript, Astronomy Engine, and independent JPL DE440s/Skyfield. Downloads and isolated Python dependencies stay in ignored `hko-cache/`; output files cannot be overwritten. See `docs/mingli/validation/hko-2057-resolution.md` for reproduction and the limited conclusion: this is a documented near-midnight sensitive boundary, not extraction error; the exact historical HKO model remains unknown.
