...
private final class CaptureProtocol:URLProtocol {
 ...
 override func startLoading(){
  do {
   ...
   try data.write(to:URL(fileURLWithPath:"/tmp/suji-b3-actual-chatclient-wire.json"))
...
@main struct Capture{
 static func main()async throws{
  ...
  let data=try Data(contentsOf:URL(fileURLWithPath:"/tmp/suji-b3-capacity-request-\(index).json"));let messages=try JSONDecoder().decode([ChatMessage].self,from:data)
