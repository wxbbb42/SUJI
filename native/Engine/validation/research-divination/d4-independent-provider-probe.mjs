...
const rows=JSON.parse(await readFile('/tmp/suji-d4-native-capacity-final.json','utf8'));
for(const row of rows){
  const encoded=JSON.parse(await readFile(`/tmp/suji-d4-capacity-request-${row.index}.json`,'utf8'));
...
await writeFile('/tmp/suji-d4-native-capacity-provider-report.json',JSON.stringify(rows,null,2)+'\n');
