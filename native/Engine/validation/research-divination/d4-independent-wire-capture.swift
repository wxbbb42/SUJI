...
private final class CaptureProtocol:URLProtocol {
 ...
 override func startLoading(){
  do {
   ...
   try data.write(to:URL(fileURLWithPath:"/tmp/suji-d4-actual-chatclient-wire.json"))
...
@main struct Capture{
 static func main()async throws{
  let data=try Data(contentsOf:URL(fileURLWithPath:"/tmp/suji-d4-capacity-request-0.json"));let messages=try JSONDecoder().decode([ChatMessage].self,from:data)
  ...
  print("Captured fixture0 through actual ChatClient without network: \(result)")
 }
}
