...
For the actual native reading session, first run Xcode `build-for-testing` with the existing production public configuration, then run `node supabase/scripts/smoke-native-reading.mjs --create-test-user --xctestrun <generated.xctestrun> --output <new-evidence-directory> --device <simulator-udid> --cases 02-pillars,24-pokemon`. This opt-in test performs native password login, dossier creation, real reading requests through the production native verifier, follow-up/retry and persistence checks using synthetic data. It creates and removes a disposable account, keeps its credentials in a temporary mode-0600 file, and never uses a real user's notebook. The output contains synthetic answers/receipts and must stay in ignored artifacts. Ordinary CI skips the live test without its private configuration. A nonzero result or remaining cleanup ID requires attention; successful HTTP responses and verifier acceptance do not establish semantic correctness or prediction accuracy.

For all 32 independent questions, use `node supabase/scripts/run-reading-matrix.mjs --create-test-user --xctestrun <generated.xctestrun> --output <new-evidence-directory> --device <simulator-udid>`. It runs sequential batches of six with at least 6.2 seconds between model requests and at most 100 requests per disposable batch; it does not modify quota limits or deploy anything. The project-wide usage counter retains test usage. Optional `--cases` selects a bounded subset. Then run `node supabase/scripts/summarize-reading-matrix.mjs <evidence-directory>` and manually compare each answer with its actual receipts. The observer forwards real responses but buffers SSE, so this does not validate live token latency or physical-device UI. See [the matrix acceptance report](../docs/mingli/validation/reading-matrix-2026-09-22.md) for observed failures and source-bound limitations. Do not describe the matrix as passed when any recorded case or semantic review failed.
